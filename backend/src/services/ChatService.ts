...
export class ChatService {
  ...
  private async buildContext(
    ...
    const activeRecs =
      await this.recommendationRepo.findActiveByUser(userId);

    const activeRecommendations = activeRecs.map(
      (r: { title: string; category: string }) => ({
        title: r.title,
        category: r.category,
      })
    );
  ...
  private formatTopFactors(
    ...
    const factors = shapRows.map((row: ShapExplanation) =>
      row.featureName
        .replace(/([A-Z])/g, ' $1')
        .toLowerCase()
    );
  ...
  private readonly intentHandlers: {
  ...
  }[] = [
    {
      ...
      handle: (ctx) =>
        ctx.prediction
          ? `Hello. Your latest burnout risk is ${
              ctx.prediction.riskLevel
            } at ${(ctx.prediction.riskScore * 100).toFixed(
              0
            )}%. ${this.formatTopFactors(
              ctx.topShapRows
            )} Ask me about your recommendations, sleep, workload, or the wellness resources if you'd like more detail.`
          : 'Hello. Complete a check-in first and I can personalize your burnout guidance with your latest prediction.',
    },

    {
      pattern:
        /(what.?if|what if|simulate|simulation|counterfactual)/,
      handle: () =>
    ...
    {
      pattern:
        /(journal|reflect|reflection|write about my day)/,
      handle: () =>
    ...
    {
      pattern:
        /(wellness|resource|article|breathing|meditation|counsel|counseling|therapist|help me find)/,
      handle: async () => {
        const resources =
          await this.resourceRepo.findAllActive();
    ...
    {
      pattern:
        /(recommend|advice|suggest|help me|action plan|what should i do)/,
      handle: (ctx) => {
        if (ctx.activeRecommendations.length > 0) {
          const list = ctx.activeRecommendations
            .slice(0, 3)
            .map((r) => r.title)
            .join('; ');
        ...
        return ctx.prediction
          ? `Based on your current data, I would focus on the top drivers behind your ${ctx.prediction.riskLevel.toLowerCase()} risk. ${this.formatTopFactors(
              ctx.topShapRows
            )} Check the Recommendations page for a full personalized action plan.`
    ...
    {
      pattern:
        /(meeting|meetings|context switch|sprint pressure|on.?call|urgent task|bug.?fixing|deadline)/,
      handle: (ctx) =>
        ctx.prediction
          ? `Work-pattern factors like meeting load, sprint pressure, and context switching can meaningfully affect burnout risk. ${this.formatTopFactors(
              ctx.topShapRows
            )} If these feel unmanageable, the "request manager support" and "reduce meeting overload" recommendations are designed for exactly this.`
    ...
    {
      pattern:
        /(risk|score|prediction|burnout)/,
      handle: (ctx) =>
        ctx.prediction
          ? `Your latest burnout risk is ${
              ctx.prediction.riskLevel
            } with a score of ${(ctx.prediction.riskScore * 100).toFixed(
              0
            )}%. ${this.formatTopFactors(
              ctx.topShapRows
            )}`
    ...
    {
      pattern:
        /(sleep|rest|tired|insomnia)/,
      handle: (ctx) =>
        ctx.prediction
          ? `Sleep is one of the biggest factors in most burnout profiles, including yours. ${this.formatTopFactors(
              ctx.topShapRows
            )} A consistent bedtime and reducing screen time 45 minutes before sleep are two of the highest-impact changes you can make.`
          : 'Sleep quality matters a lot for burnout risk. Once you have a prediction, I can tailor advice to your data.',
    },

    {
      pattern:
        /(stress|overwhelm|pressure|anxious|anxiety)/,
      handle: () =>
        'When stress or anxiety is high, short breathing exercises and reducing workload where possible can help. If this feeling persists, consider reaching out to a trusted person or appropriate professional support.',
    },

    {
      pattern:
        /(exercise|walk|physical activity|workout)/,
      handle: () =>
        'Even short activity, like a 20-minute walk during lunch, can be a useful way to manage stress. Check the Exercise section of Wellness Resources for no-equipment routines designed for people who sit most of the day.',
    },

    {
      pattern:
        /(what is burnout|define burnout|burnout mean)/,
      handle: () =>
...

    {
      ...
      handle: () =>
        "You're welcome. I'm here anytime you want to check your risk, recommendations, or find a wellness resource.",
    },

    {
      pattern: /(bye|goodbye|see you)/,
      handle: () =>
        'Take care of yourself. Come back anytime you want to check in on your wellbeing.',
    },
